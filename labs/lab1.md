...
### Вариант 1. Карта загрузки

Запустите последовательный терминал с временными метками и записью журнала:

```bash
tio -b 115200 -t -L --log-file boot.log /dev/ttyUSBX
```

Выполните штатную перезагрузку платы. Найдите в `boot.log` по одной характерной строке для загрузчика U-Boot, ядра Linux и пользовательского пространства (`systemd` или приглашение `login:`). Для каждой строки объясните, какие компоненты системы к этому моменту уже работают. После сохранения нужного фрагмента отчёта удалите журнал.

### Вариант 2. Паспорт платы

Составьте таблицу «характеристика — команда — значение — интерпретация» для архитектуры, числа процессоров, версии ядра, объёма RAM, разделов, файловых систем, сетевых интерфейсов, имени системы и часового пояса. Используйте:
...
Не устанавливайте дополнительные утилиты только ради этой таблицы. Если `fdisk` отсутствует в минимальном образе, сведений `lsblk -f` достаточно.

### Вариант 3. Сравнение двух компьютеров

Выполните следующие команды сначала на Lichee RV Dock, а затем на рабочем компьютере:
...
Подпишите, к какой системе относится каждый блок вывода. Объясните не менее пяти различий, разделив их на архитектурные, ресурсные и связанные с назначением системы.

### Вариант 4. Устойчивое имя UART

На рабочем компьютере определите модель и идентификатор USB-UART-преобразователя:
...
Найдите символьную ссылку, которая ведёт на используемый `/dev/ttyUSBX`. Подключитесь через путь из `/dev/serial/by-id/` и объясните, почему он надёжнее динамического номера `ttyUSBX`.

### Вариант 5. Наблюдение нагрузки

На плате зафиксируйте начальную нагрузку, запустите контролируемый вычислительный процесс и сохраните его PID:
...
Выйдите из `top` клавишей `q`, завершите только созданный процесс и проверьте очистку:
...
Команда `ps` после успешной очистки не должна найти процесс. Не используйте `killall yes`: на общем стенде он может завершить чужой процесс.

### Вариант 6. Диагностика скорости UART

Закройте рабочий сеанс `tio`, подключитесь к тому же устройству с заведомо неверной скоростью 9600 бод и по указанию преподавателя перезагрузите плату:

```bash
tio -b 9600 /dev/ttyUSBX
```

Зафиксируйте искажённый вывод, не меняя другие параметры и соединения. Закройте терминал, верните скорость 115200 и убедитесь, что журнал снова читается:

```bash
tio -b 115200 /dev/ttyUSBX
```

Объясните, почему нечитаемые символы при неверной скорости указывают на наличие сигнала, а не на полное отсутствие связи.
