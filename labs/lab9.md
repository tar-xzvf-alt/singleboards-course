...
::: additional
Логический анализатор различает дискретные уровни, удобен для длительного многоканального захвата и декодирования протоколов. Осциллограф показывает реальную форму и амплитуду сигнала, поэтому лучше подходит для поиска выбросов, фронтов, шумов и нарушений электрических уровней.
:::
...
::: attention
Перед подключением обесточьте плату и анализатор. Соедините их GND и подключайте к плате только входной канал анализатора, рассчитанный на уровни 3,3 В; не соединяйте между собой два выхода и не подавайте питание с анализатора на сигнальную линию.
:::
...
![Декодирование загрузочного вывода UART 115200 8N1 в PulseView](../pictures/UART_output.png){width=100%}
...
::: additional
Пакет `libgpiod2` содержит библиотеку времени выполнения, необходимую уже собранным программам. Пакет `libgpiod-devel` предоставляет заголовки и файлы для компоновки, необходимые при сборке программы с `-lgpiod`.
:::
...
::: attention
Перед использованием PE16 выполните `gpioinfo` и убедитесь, что на данной системе PE16 соответствует линии 144 устройства `/dev/gpiochip0` и не занят другим потребителем. Подключайте анализатор при выключенном питании и не соединяйте PE16, настроенный как выход, с другим активным выходом: конфликт уровней может повредить оборудование.
:::
