...
## Вариант 2. Пульт управления светодиодами через MQTT и SPI
...
1. **Arduino — приём команд по SPI (лаб. №10, №11).** Прошивка работает в режиме SPI-slave и управляет четырьмя светодиодами на пинах 3, 5, 6 и 9 мезонинной платы. Пин 10 зарезервирован аппаратным SPI как вход SS и для управления светодиодом не используется. Команды приходят от Lichee-мастера. Протокол команд студент проектирует самостоятельно, например байт команды и байт данных или четыре бита для четырёх светодиодов.
...
## Вариант 3. Панель системного мониторинга одноплатника
...
> [!TIP]
> **Дополнительное сравнение.** C-сборщик в варианте 3 выполняется после готовой Python-версии. Используйте одинаковый набор метрик и интервал публикации, отдельно зафиксируйте размер процесса, загрузку CPU и размер исполняемого файла.
...
- Частота и скважность, измеренные в PulseView, соответствуют заданным (погрешность в пределах 5% во всём заданном диапазоне)
...
```text
ПК (GUI-редактор) ──MQTT──▶ Lichee RV Dock ──SPI(master)──▶ Arduino Uno ──GPIO──▶ 4 LED
```
...
## Вариант 9. Самописец данных с хранением на SD-карте
...
```text
ПК (GUI) ──MQTT──▶ Lichee ──SPI-команды + polling──▶ Arduino (LED + кнопка)
                       ◀────────SPI-ответ──────────────┘
                       │
                       ▼ MQTT
                    ПК (GUI)
...
1. **Arduino — SPI-slave + локальное прерывание (лаб. №10, №11).** Прошивка:
