...
![Многоточечная топология шины I2C с линиями SDA и SCL](../pictures/i2c.png){width=86%}
...
::: additional
TWI (Two-Wire Interface) — распространённое нейтральное название аппаратного контроллера, совместимого с шиной I2C; в документации Allwinner обозначения TWI2 и I2C2 относятся к одному интерфейсу.
:::

![Распиновка Lichee RV Dock: линии TWI2_SDA и TWI2_SCL](../pictures/pinout.png){width=100%}
...
::: attention
Не копируйте шаблоны DTS без проверки: `НОМЕРА ПИНОВ ДЛЯ ИНТЕРФЕЙСА` ниже — учебный placeholder, который нужно заменить реальными выводами по схеме и документации SoC. Строка `#compatible` не является допустимым свойством Device Tree и в конфигурации использоваться не должна.
:::
...
После выполнения всех шагов в каталоге */dev/* появляется символьное устройство `/dev/i2c-2`. Для проверки работоспособности интерфейса можно выполнить тестирование путем запуска программы вывода собственного имени и фамилии студента на экран SSD-1306:

::: attention
Номер адаптера должен быть согласован во всех шагах: для активированного `i2c2` проверьте наличие `/dev/i2c-2` и используйте `port=2` в программе. Если фактическая нумерация в вашей сборке отличается, сначала определите соответствие через `i2cdetect -l`, а затем одинаково скорректируйте имя устройства и параметр `port`.
:::
...
::: additional
Если файл DejaVuSans отсутствует, `ImageFont.load_default()` позволяет скрипту продолжить работу с резервным шрифтом. Он может не содержать кириллицу или иметь меньший размер, поэтому при некорректном отображении установите шрифт с поддержкой кириллицы и укажите путь к нему.
:::

::: attention
Подключайте SSD1306 только при полностью выключенном питании платы: VCC дисплея — к 3,3 В, GND — к GND, SDA — к TWI2_SDA, SCL — к TWI2_SCL. Проверьте маркировку модуля и обязательно обеспечьте общий GND; не допускайте попадания 5 В на SDA или SCL.
:::

Для работы с I2C-экраном подключите его к питанию 3,3 В и линиям TWI2 в соответствии с распиновкой платы.

**В отчёте зафиксируйте вывод фамилии и имени студента на дисплей.**
