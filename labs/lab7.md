...
![Топология SPI с общими линиями данных и отдельными сигналами выбора ведомых устройств](../pictures/SPImult.png){width=58%}
...
::: additional
Полнодуплексный обмен означает, что передача по MOSI и приём по MISO происходят одновременно на каждом такте SCK. Поэтому вызов обмена SPI всегда возвращает столько же байтов, сколько было передано.
:::
...
![Подключение одного SPI-устройства к Lichee RV Dock](../pictures/SPI.png){width=68%}
...
![Распиновка Lichee RV Dock: сигналы SPI1 на выводах PD10-PD15](../pictures/pinout.png){width=100%}
...
::: attention
Выводы PD10-PD15 имеют альтернативные RGB-функции, поэтому перед включением SPI убедитесь, что они не заняты дисплеем или другим узлом Device Tree. Любые внешние соединения с этими выводами выполняйте только при выключенном питании платы.
:::
...
::: additional
SPIDEV можно привязать через допустимый идентификатор `compatible` из таблицы драйвера либо добавить идентификатор реального устройства в драйвер и пересобрать ядро. Для уже созданного SPI-устройства также существует привязка через `driver_override`, но в этой работе используется вариант с Device Tree.
:::
...
::: additional
Название `spi-sun6i` историческое: драйвер появился для более ранних SoC Allwinner, а затем получил таблицы совместимости и особенности новых семейств, включая sun20i. Имя параметра `SPI_SUN6I` поэтому не означает, что драйвер предназначен только для sun6i.
:::
...
::: attention
Рабочая конфигурация требует полного комплекта изменений: активного узла SPI в Device Tree и параметров ядра `SPI_SPIDEV`, `SPI_SUN6I` и `DMA_SUN6I`. Пропуск любого из этих компонентов может привести к отсутствию `/dev/spidev*` или ошибке инициализации контроллера.
:::
...
::: attention
Перед установкой перемычки MOSI-MISO полностью отключите питание платы. Соединяйте только указанные сигнальные выводы; они работают с логическими уровнями 3,3 В. Не подавайте на них 5 В и не замыкайте их на линии питания.
:::
...
![Результат полнодуплексного теста SPI с перемычкой между MOSI и MISO](../pictures/SPI_result.png){width=88%}
