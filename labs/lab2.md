...
::: additional
SSH (Secure Shell) — криптографический сетевой протокол для безопасного удалённого выполнения команд, туннелирования соединений и авторизации по ключам. SCP (Secure Copy Protocol) использует SSH для защищённого копирования файлов между системами.
:::
...
::: attention
Для входа по паролю раскомментируйте `PasswordAuthentication` и задайте нужное значение. Параметр `PermitRootLogin yes` разрешает удалённый вход суперпользователя и должен использоваться только при явной необходимости:
...
После изменения конфигурации **обязательно** перезапустите службу `sshd`, иначе новые параметры не вступят в силу:
...
::: attention
После первичной настройки создайте отдельного пользователя с собственным паролем и выполняйте дальнейшую работу по SSH через эту учётную запись, а не через `root`.
:::
