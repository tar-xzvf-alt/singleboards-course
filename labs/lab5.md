...
::: additional
Если подходящего DTS нет, его можно подготовить самостоятельно по документации SoC, схеме и распиновке платы. Практической отправной точкой часто служит DTS максимально похожего устройства, который адаптируют под нужное оборудование.
:::
...
::: additional
Вместо самостоятельной сборки можно использовать готовый U-Boot с OpenSBI версии 1.7 из [репозитория ALT Linux](https://packages.altlinux.org/ru/sisyphus_riscv64/binary/u-boot-sunxi-riscv/noarch/3229656311355752582).
:::

Для этого переключимся на репозиторий `sisyphus_riscv64`, если сейчас он не выбран.

::: attention
Следующие действия полностью заменяют содержимое `/etc/apt/sources.list`. Перед заменой сохраните текущую конфигурацию репозиториев, чтобы затем её восстановить.
...
Так вот **extlinux.conf** на одноплатнике — это простой текстовый файл, который сообщает загрузчику U-Boot следующую информацию:

- Расположение ядра или сжатого ядра
- Какой Device Tree Blob (.dtb) файл использовать
- Параметры командной строки, с которыми запустить выбранное ядро

::: attention
Параметры `kernel`, `fdt` и `root` в `extlinux.conf` критичны для загрузки: они должны указывать соответственно на фактические пути к ядру и DTB и на раздел с корневой файловой системой. Ошибка в любом из этих значений остановит загрузку системы.
:::
...
::: attention
Сохраните подготовленные ядро, DTB, загрузчик, `extlinux.conf` и архив rootfs: все эти компоненты потребуются при создании образа в следующей лабораторной работе.
:::
