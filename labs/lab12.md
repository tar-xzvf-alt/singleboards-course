...
::: additional
Retained-сообщение хранит последнее состояние топика для новых подписчиков, а Will сообщает об аварийном отключении клиента. В дополнительной части можно добавить retained-статус `online` при запуске и will-статус `offline` при потере соединения.
:::
...
::: additional
Команда `hostname -i` может вывести несколько адресов. Выберите адрес интерфейса, находящегося в одной учебной сети с ПК, и подтвердите его командами `ip address` и `ping <IP_LICHEE>`.
:::
...
::: attention
Перед запуском подписчика замените `<IP_LICHEE>` фактическим адресом Lichee и проверьте с ПК доступность порта 1883, например командой `telnet <IP_LICHEE> 1883`. Не продолжайте с демонстрационным или недоступным адресом.
:::
...
::: attention
Соблюдайте порядок запуска: сначала брокер Mosquitto, затем издатель на Lichee и только после этого подписчик на ПК. Перед каждым следующим этапом убедитесь, что предыдущий запущен без ошибок.
:::
...
::: additional
C-клиент является дополнительной реализацией издателя. Перед его сборкой сначала проверьте базовую цепочку брокер → Python-издатель → подписчик, затем замените только издатель и сравните формат сообщений и устойчивость подключения.
:::
