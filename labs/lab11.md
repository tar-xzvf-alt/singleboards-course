...
> *Схема обмена данными между компонентами системы — здесь необходимо вставить наглядную блок-схему, иллюстрирующую цепочку передачи данных: BME280 → I2C → Arduino → SPI → Lichee → I2C → OLED.*
...
Готовые файлы для этой лабораторной доступны в репозитории:

- [Ардуино-скетч SPI-слейва](../examples/arduino_i2c_spi/arduino_spi_slave/arduino_spi_slave.ino) — реализация SPI-slave на ATmega328P с использованием регистров SPCR/SPDR/SPIF
- [Python-скрипт приёма SPI и вывода на OLED](../examples/lichee_integration/lichee_spi_oled.py) — чтение данных через `/dev/spidev1.0`, распаковка `struct.unpack` и отображение на SSD-1306
